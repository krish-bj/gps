...
def _calc_dist_meters(lat1: float, lng1: float, lat2: float, lng2: float) -> float:
    lat_mid = math.radians((lat1 + lat2) / 2.0)
    dx = math.radians(lng2 - lng1) * math.cos(lat_mid) * 6371000.0
...
class VehicleSimulator:
    ...
    """
    def __init__(self, vehicle_code: str, waypoints: list, speed_base: float = 42.0):
    ...
    def get_next_telemetry(self) -> dict:
        ...
        p2 = self.waypoints[(self.current_segment + 1) % num_waypoints]
        ...
        # Advance along route based on real physical distance for interval
        dist_to_move = (speed / 3.6) * SIMULATION_INTERVAL  # in meters

        while dist_to_move > 0 and num_waypoints > 1:
            p_cur = self.waypoints[self.current_segment]
            p_next = self.waypoints[(self.current_segment + 1) % num_waypoints]
            seg_len = _calc_dist_meters(p_cur["lat"], p_cur["lng"], p_next["lat"], p_next["lng"])

            if seg_len <= 0.001:
                self.current_segment = (self.current_segment + 1) % num_waypoints
                self.progress = 0.0
                continue

            remaining_in_seg = seg_len * (1.0 - self.progress)
            if dist_to_move < remaining_in_seg:
                self.progress += dist_to_move / seg_len
                dist_to_move = 0
            else:
